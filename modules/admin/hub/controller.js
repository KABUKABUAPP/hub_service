...
exports.addNewHubController = async (req, res, next) => {
  try {
    const payload = {
      ...
      opening_hours: String(req.body?.opening_hours).toLowerCase(),
      city: String(req.body.city).toLowerCase(),
      state: String(req.body?.state).toLowerCase(),
      inspector: req.body?.inspector
        ? String(req.body?.inspector).toLowerCase()
        : undefined,
...
exports.getAllHubsController = async (req, res, next) => {
  try {
    ...
    console.log(
      "🚀 ~ file: controller.js:82 ~ exports.getAllHubsController= ~ searchField:",
      searchField
    );
    ...
    console.log(
      "🚀 ~ file: controller.js:99 ~ exports.getAllHubsController= ~ payload:",
      payload
    );
